...
(function() {
    ...
    function boot() {
        ...
        if (dropZone && apkInput) {
            ...
            apkInput.addEventListener('change', handleFile);
            dropZone.addEventListener('dragover', e => { e.preventDefault(); dropZone.style.borderColor = 'var(--prestige-gold)'; });
            dropZone.addEventListener('dragleave', () => dropZone.style.borderColor = 'var(--prestige-sand)');
        ...
        function handleFile() {
            if (apkInput.files && apkInput.files[0]) {
                ...
                toast('APK prêt pour analyse');
            }
        }

        const btnScanEl = document.getElementById('btn-scan');
        if (btnScanEl) btnScanEl.addEventListener('click', startScan);
    ...
    // ── Scanner ──────────────────────────────────────────────────────────────
    function startScan() {
        if (!state.currentFile) { toast('Sélectionnez un APK d\'abord'); return; }

        const data = {
            deviceName:      state.currentFile.name,
            fileSize:        (state.currentFile.size / (1024 * 1024)).toFixed(2) + ' MB',
            // Mocking APK features for the engine
            isApk:           true,
            hasNativeCode:   true,
            permissionsCount: 12
        };
        ...
        showView('scanning');
        setText('scan-pct', '0%');
        setText('scan-msg', 'Extraction du Manifest...');
        document.getElementById('scan-bar').style.width = '0%';

        let p = 0;
        const msgs = ['Décompression APK...','Analyse du Manifest...','Contrôle des Signatures...','Analyse des Permissions...','Détection de Malware...','Génération Rapport...'];
        const iv = setInterval(() => {
            p += 2;
            document.getElementById('scan-bar').style.width = p + '%';
            setText('scan-pct', p + '%');
            setText('scan-msg', msgs[Math.floor(p / 16.6)] || 'Finalisation...');
